...
class Settings(BaseSettings):
    ...
    def __init__(self, **data: Any):
        ...
        if "llm" in config:
            ...
            data.setdefault("llm_timeout_seconds", config["llm"].get("timeout_seconds", 120))
            data.setdefault("llm_daily_budget_usd", config["llm"].get("daily_budget_usd", 10.0))
            data.setdefault("llm_max_events_per_msg", config["llm"].get("max_events_per_msg", 5))
        
        if "database" in config:
            data.setdefault("db_path", config["database"].get("path", "data/slack_events.db"))
        
        if "slack" in config:
            data.setdefault("slack_digest_channel_id", config["slack"].get("digest_channel_id", "YOUR_DIGEST_CHANNEL_ID"))
            data.setdefault("lookback_hours_default", config["slack"].get("lookback_hours_default", 24))
        
        if "processing" in config:
            data.setdefault("tz_default", config["processing"].get("tz_default", "Europe/Amsterdam"))
            data.setdefault("threshold_score_default", config["processing"].get("threshold_score_default", DEFAULT_THRESHOLD_SCORE))
        
        if "deduplication" in config:
            data.setdefault("dedup_date_window_hours", config["deduplication"].get("date_window_hours", DEFAULT_DATE_WINDOW_HOURS))
            data.setdefault("dedup_title_similarity", config["deduplication"].get("title_similarity", DEFAULT_TITLE_SIMILARITY))
            data.setdefault("dedup_message_lookback_days", config["deduplication"].get("message_lookback_days", DEFAULT_MESSAGE_LOOKBACK_DAYS))
    ...
    llm_temperature: float = Field(default=1.0, description="LLM temperature (1.0 for gpt-5-nano)")
